...
This tool is designed for the Center for Complex Network Research (CCNR) at Northeastern University by Brian Ward, under Babak Ravandi. This tool was designed with the ultimate goal to find similar nodes of the FoodOn database, which holds many (~20k) nodes representing food items and food processes. MORE HERE


## Tool Methods

1. **ProcessStrings** : preprocessing tool for labels.
2. **Sim**: calculate similarity value between two labels
3. **SimAll** : calculate similarity value between each label given. 
4. **trainModel**: creates a Gensim Word2Vec Model  for label comparison.
